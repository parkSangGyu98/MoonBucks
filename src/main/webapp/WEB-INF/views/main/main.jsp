...
<body class="d-flex h-100 text-center text-white bg-dark">
	...
	<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
		...
		<main style="margin-left: -180px; width: 1000px;" class="container">
			<form id="aTag" action="main" method="post" style="margin:0;">
				<h1 style="margin-top: 30px;">Category</h1>
				<div class="row mb-2">
					<c:forEach var="x" items="${category}" varStatus="status">
						<div style="margin-bottom: 30px;" class="col-md-6">
							<div
								class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
								<img src="../resources/img/category/${x.picture}.jpg">
							</div>
							<input type="submit" name="name" value="${x.name}">
						</div>
					</c:forEach>
				</div>
			</form>
						...
						<ul>
							<c:forEach var="item" items="${order}" varStatus="status">
								<c:if test="${order != null}">
									<li style="height: 50px; line-height:100px;">주문${status.index+1}) ${item.categoryName} - ${item.name} - ${item.size} - ${item.count} - ${item.take}</li>
								</c:if>
							</c:forEach>
						</ul>
					</div>
